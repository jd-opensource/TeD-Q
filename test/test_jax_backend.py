#   Copyright 2021-2024 Jingdong Digits Technology Holding Co.,Ltd.
#
#   Licensed under the Apache License, Version 2.0 (the "License");
#   you may not use this file except in compliance with the License.
#   You may obtain a copy of the License at
#
#       http://www.apache.org/licenses/LICENSE-2.0
#
#   Unless required by applicable law or agreed to in writing, software
#   distributed under the License is distributed on an "AS IS" BASIS,
#   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#   See the License for the specific language governing permissions and
#   limitations under the License.


#TODO: add tests for cotengra and cyc_cotengra
r"""
Test file for storage and circuit.
"""
import numpy as np
import random
import pytest
from jax import numpy as jnp
import torch
import jax

import tedq as qai
from tedq.QInterpreter.operators.ops_abc import GateBase
from tedq.QInterpreter.operators.measurement import QuantumMeasurement
from tedq.QInterpreter.circuits.storage_base import StorageBase, CircuitStorage
from tedq.QInterpreter.circuits.circuit import Circuit
from tedq.quantum_error import QuantumStorageError
from tedq.backends.jax_backend import JaxBackend

class Test_jax_backend():
    r"""
    """
    def test_construction(self):
        params = tuple([random.random() for _ in range(2)])
        def circuitDef(*params):
            qai.RY(params[0], qubits=[0])
            qai.RZ(params[1], qubits=[1])
            return qai.expval(qai.PauliZ(qubits=[0]))

        circuit = qai.Circuit(circuitDef, 2, *params)

        compiled_circuit = circuit.compilecircuit(backend="jax")
        assert isinstance(compiled_circuit, JaxBackend)
        assert compiled_circuit.interface == "jax"
        assert compiled_circuit.diff_method == "back_prop"
        assert compiled_circuit.backend == "jax"

        compiled_circuit = circuit.compilecircuit(backend="jax", interface="pytorch")
        assert isinstance(compiled_circuit, JaxBackend)
        assert compiled_circuit.interface == "pytorch"
        assert compiled_circuit.diff_method == "back_prop"
        assert compiled_circuit.backend == "jax"

        print("Test jax backend construction ok!")

    def test_unknown_interface(self):
        r"""
        """
        params = tuple([random.random() for _ in range(2)])
        def circuitDef(*params):
            qai.RY(params[0], qubits=[0])
            qai.RZ(params[1], qubits=[1])
            return qai.expval(qai.PauliZ(qubits=[0]))

        circuit = qai.Circuit(circuitDef, 2, *params)

        with pytest.raises(ValueError):
            compiled_circuit = circuit.compilecircuit(backend="jax", interface="arbitrary")

        print("Test unknown interface ok!")

    def test_confliction_order_finder(self):
        """
        """
        params = tuple([random.random() for _ in range(2)])
        def circuitDef(*params):
            qai.RY(params[0], qubits=[0])
            qai.RZ(params[1], qubits=[1])
            return qai.expval(qai.PauliZ(qubits=[0]))

        circuit = qai.Circuit(circuitDef, 2, *params)

        with pytest.raises(ValueError, match="Error!!!! can not use contengra, opt_einsum and cyc at the same time!"):
            compiled_circuit = circuit.compilecircuit(backend="jax",use_cotengra=True, use_jdopttn=True)

        print("Test confliction on two contraction order finder ok!")

    def test_tensor_data(self):
        """
        """
        Hadamard = qai.Hadamard(qubits=[0], do_queue = False)
        np_matrix_a = Hadamard.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_Hadamard_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        PauliX = qai.PauliX(qubits=[1], do_queue = False)
        np_matrix_a = PauliX.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_PauliX_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        PauliY = qai.PauliY(qubits=[2], do_queue = False)
        np_matrix_a = PauliY.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_PauliY_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        PauliZ = qai.PauliZ(qubits=[3], do_queue = False)
        np_matrix_a = PauliZ.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_PauliZ_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        I = qai.I(qubits=[4], do_queue = False)
        np_matrix_a = I.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_I_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        S = qai.S(qubits=[5], do_queue = False)
        np_matrix_a = S.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_S_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        T = qai.T(qubits=[6], do_queue = False)
        np_matrix_a = T.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_T_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        SX = qai.SX(qubits=[7], do_queue = False)
        np_matrix_a = SX.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_SX_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        #print(np_matrix)
        #print(jnp_matrix)
        assert (np_matrix == jnp_matrix).all()

        CNOT = qai.CNOT(qubits=[0, 1], do_queue = False)
        np_matrix_a = CNOT.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CNOT_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        CY = qai.CY(qubits=[1, 2], do_queue = False)
        np_matrix_a = CY.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CY_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        CZ = qai.CZ(qubits=[2, 3], do_queue = False)
        np_matrix_a = CZ.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CZ_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        SWAP = qai.SWAP(qubits=[3, 4], do_queue = False)
        np_matrix_a = SWAP.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_SWAP_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        CSWAP = qai.CSWAP(qubits=[1, 2, 0], do_queue = False)
        np_matrix_a = CSWAP.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CSWAP_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        Toffoli = qai.Toffoli(qubits=[1, 2, 0], do_queue = False)
        np_matrix_a = Toffoli.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_Toffoli_tensor(paramslist=[])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        RX = qai.RX(0.5, qubits=[0], do_queue = False)
        np_matrix_a = RX.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_RX_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        RY = qai.RY(0.5, qubits=[1], do_queue = False)
        np_matrix_a = RY.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_RY_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        RZ = qai.RZ(0.5, qubits=[2], do_queue = False)
        np_matrix_a = RZ.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_RZ_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        Rot = qai.Rot(0.3, 0.4, 0.5, qubits=[0], do_queue = False)
        np_matrix_a = Rot.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_Rot_tensor(paramslist=[0.3, 0.4, 0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        #print(np_matrix)
        #print(jnp_matrix)
        assert (np_matrix == jnp_matrix).all()

        PhaseShift = qai.PhaseShift(0.5, qubits=[0], do_queue = False)
        np_matrix_a = PhaseShift.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_PhaseShift_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        ControlledPhaseShift = qai.ControlledPhaseShift(0.5, qubits=[1, 0], do_queue = False)
        np_matrix_a = ControlledPhaseShift.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_ControlledPhaseShift_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        CRX = qai.CRX(0.5, qubits=[0, 1], do_queue = False)
        np_matrix_a = CRX.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CRX_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        CRY = qai.CRY(0.5, qubits=[0, 1], do_queue = False)
        np_matrix_a = CRY.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CRY_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        CRZ = qai.CRZ(0.5, qubits=[0, 1], do_queue = False)
        np_matrix_a = CRZ.matrix
        np_matrix_b = jnp.asarray(np_matrix_a, dtype = jnp.complex64).reshape([2,2,2,2])
        np_matrix = jnp.round(np_matrix_b, decimals = 5)
        jnp_matrix_a = JaxBackend.get_CRZ_tensor(paramslist=[0.5])
        jnp_matrix = jnp.round(jnp_matrix_a, decimals = 5)
        assert (np_matrix == jnp_matrix).all()

        print("Test tensor data ok!")

    def test_init_state(self):
        """
        """
        def circuitDef(*params):
            return qai.state()

        num_qubits = random.randint(1, 10)
        circuit = qai.Circuit(circuitDef, num_qubits)
        my_compilecircuit = circuit.compilecircuit(backend="jax")
        results = my_compilecircuit()
        state = results[0]
        state = jnp.round(state, decimals = 5)
        num_elements = 2 ** num_qubits
        array = [0. for _ in range(num_elements)]
        array[0] = 1.
        state_tensor = jnp.array(array, dtype=jnp.complex64)
        shape = [2 for _ in range(num_qubits)]
        state_tensor = state_tensor.reshape(shape)
        state_tensor = jnp.round(state_tensor, decimals = 5)
        assert (state == state_tensor).all()

        print("Test state initialization ok!")

    def test_SVPM_forward_jax_interface(self):
        """
        """
        def circuitDef(*params):
            qai.RX(params[0], qubits=[0])
            qai.RY(params[1], qubits=[0])
            return [qai.expval(qai.PauliZ(qubits=[0])), qai.probs(), qai.probs(qubits=[0]), qai.state()]

        num_qubits = 2
        params = (0.54, 0.12)
        circuit = qai.Circuit(circuitDef, num_qubits, *params)
        my_compilecircuit = circuit.compilecircuit(backend="jax")
        results = my_compilecircuit(*params)

        expval_result = results[0]
        expval_result = jnp.round(expval_result, decimals = 5)
        expval_correct = jnp.array([0.85154057], dtype = jnp.float32)
        expval_correct = jnp.round(expval_correct, decimals = 5)
        assert (expval_result == expval_correct).all()

        all_probs_result = results[1]
        all_probs_result = jnp.round(all_probs_result, decimals = 5)
        all_probs_correct = jnp.array([[0.9257702, 0.], [0.07422972, 0.]], dtype = jnp.float32)
        all_probs_correct = jnp.round(all_probs_correct, decimals = 5)
        assert (all_probs_result == all_probs_correct).all()

        part_probs_result = results[2]
        part_probs_result = jnp.round(part_probs_result, decimals = 5)
        part_probs_correct = jnp.array([0.9257702, 0.07422972], dtype = jnp.float32)
        part_probs_correct = jnp.round(part_probs_correct, decimals = 5)
        assert (part_probs_result == part_probs_correct).all()

        state_result = results[3]
        state_result = jnp.round(state_result, decimals = 5)
        state_correct = jnp.array([[0.9620366+0.01599429j, 0.+0.j],[0.05779156-0.26625147j, 0.+0.j]], dtype = jnp.complex64)
        state_correct = jnp.round(state_correct, decimals = 5)
        assert (state_result == state_correct).all()

        print("Test jax interface state vector propagation mode forward calculation ok!")


    def test_SVPM_forward_pytorch_interface(self):
        """
        jax backend pytorch interface only support list of measurements with the same dimensions.
        TODO: can add test of list of prbs or state measurements.
        """
        def circuitDef(*params):
            qai.RX(params[0], qubits=[0])
            qai.RY(params[1], qubits=[0])
            return [qai.expval(qai.PauliZ(qubits=[0])), qai.expval(qai.PauliX(qubits=[1]))]

        num_qubits = 2
        _a = torch.tensor([0.54], dtype = torch.float32, requires_grad = True)
        _b = torch.tensor([0.12], dtype = torch.float32, requires_grad = True)
        params = (_a, _b)
        circuit = qai.Circuit(circuitDef, num_qubits, *params)
        my_compilecircuit = circuit.compilecircuit(backend="jax", interface="pytorch")
        results = my_compilecircuit(*params)

        expval_result = results[0]
        expval_result = expval_result.detach().numpy()
        expval_result = np.round(expval_result, decimals = 5)
        expval_correct = np.array([0.85154057], dtype = np.float32)
        expval_correct = np.round(expval_correct, decimals = 5)
        assert (expval_result == expval_correct).all()

        expval_result = results[1]
        expval_result = expval_result.detach().numpy()
        expval_result = np.round(expval_result, decimals = 5)
        expval_correct = np.array([0.], dtype = np.float32)
        expval_correct = np.round(expval_correct, decimals = 5)
        assert (expval_result == expval_correct).all()

        print("Test pytorch interface state vector propagation mode forward calculation ok!")


    def test_SVPM_backward_jax_interface(self):
        """
        TODO: can test each measurement separately.
        """
        def circuitDef(*params):
            qai.RX(params[0], qubits=[0])
            qai.RY(params[1], qubits=[0])
            return [qai.expval(qai.PauliZ(qubits=[0])), qai.probs(), qai.probs(qubits=[0]), qai.state()]

        num_qubits = 2
        params = (0.54, 0.12)
        circuit = qai.Circuit(circuitDef, num_qubits, *params)
        my_compilecircuit = circuit.compilecircuit(backend="jax")
        
        def cost(params):
            # my_compilecircuit takes tuple input
            # but jax.grad take list input
            params = list(params)
            # obtain results
            results = my_compilecircuit(*params)
            # seperate each measurement result
            expval_result = results[0]
            all_probs_result = results[1]
            part_probs_result = results[2]
            state_result = results[3]
            # make the output to be a scaler
            _a = jnp.tensordot(all_probs_result, state_result)
            _b = jnp.einsum("a->", part_probs_result)
            cost = -1.*_a + jnp.exp(_b) + 3.3*expval_result
            # jax.grad need real scaler output
            cost = jnp.abs(cost)
            # return the cost result
            return cost

        grad_func = jax.jit(jax.grad(jax.jit(cost)))
        # jax.grad take list input
        params = list(params)
        # obtain the gradient by jax built-in back-propagation method
        gradients = grad_func(params)
        result = jnp.array(gradients)
        result = jnp.round(result, decimals = 5)
        correct = jnp.array([-1.3297417 , -0.30148667], dtype=jnp.float32)
        correct = jnp.round(correct, decimals = 5)
        assert (result == correct).all()

        print("Test jax interface state vector propagation mode backward calculation ok!")

    def test_SVPM_backward_pytorch_interface(self):
        """
        jax backend pytorch interface only support list of measurements with the same dimensions.
        TODO: can add test of list of prbs or state measurements.
        """
        def circuitDef(*params):
            qai.RX(params[0], qubits=[0])
            qai.RY(params[1], qubits=[0])
            return [qai.expval(qai.PauliZ(qubits=[0])), qai.expval(qai.PauliX(qubits=[1]))]

        num_qubits = 2
        _a = torch.tensor([0.54], dtype = torch.float32, requires_grad = True)
        _b = torch.tensor([0.12], dtype = torch.float32, requires_grad = True)
        params = (_a, _b)
        circuit = qai.Circuit(circuitDef, num_qubits, *params)
        my_compilecircuit = circuit.compilecircuit(backend="pytorch")
        results = my_compilecircuit(*params)

        expval_result = results[0]
        expval_result.backward()
        result_grad_a = _a.grad.numpy()
        result_grad_b = _b.grad.numpy()
        result_grad_a = np.round(result_grad_a, decimals = 5)
        result_grad_b = np.round(result_grad_b, decimals = 5)
        correct_grad_a = np.array([-0.5104387], dtype=np.float32)
        correct_grad_b = np.array([-0.10267819], dtype=np.float32)
        correct_grad_a = np.round(correct_grad_a, decimals = 5)
        correct_grad_b = np.round(correct_grad_b, decimals = 5)
        assert result_grad_a == correct_grad_a
        assert result_grad_b == correct_grad_b

        # avoid backward through the graph more than one time
        # it is very important to have new input patameters,
        # otherwise the previous calculation, the grad inside the parameter tensor
        # will affect the new calculation result!
        _a = torch.tensor([0.54], dtype = torch.float32, requires_grad = True)
        _b = torch.tensor([0.12], dtype = torch.float32, requires_grad = True)
        params = (_a, _b)
        results = my_compilecircuit(*params)
        expval_result = results[1]
        expval_result.backward()
        result_grad_a = _a.grad.numpy()
        result_grad_b = _b.grad.numpy()
        result_grad_a = np.round(result_grad_a, decimals = 5)
        result_grad_b = np.round(result_grad_b, decimals = 5)
        correct_grad_a = np.array([0.], dtype=np.float32)
        correct_grad_b = np.array([0.], dtype=np.float32)
        correct_grad_a = np.round(correct_grad_a, decimals = 5)
        correct_grad_b = np.round(correct_grad_b, decimals = 5)
        assert result_grad_a == correct_grad_a
        assert result_grad_b == correct_grad_b

        print("Test pytorch interface state vector propagation mode backward calculation ok!")