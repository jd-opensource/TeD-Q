...
def entangling_layer(circ, nqubits):
...
# parameters
params_list = []
for idx in range((q_depth)*n_qubits):
    params_list.append(Parameter("param_"+str(idx)))

# parameters
input_list = []
for idx in range(n_qubits):
    input_list.append(Parameter("input_"+str(idx)))
    
print((q_depth)*n_qubits)

preparation_circ = QuantumCircuit(n_qubits, 0)


# Start from state |+> , unbiased w.r.t. |0> and |1>
H_layer(preparation_circ, n_qubits)

# Embed features in the quantum node
RY_layer(preparation_circ, input_list)


qnn_circ = QuantumCircuit(n_qubits, 0)
# Sequence of trainable variational layers
for k in range(q_depth):
    entangling_layer(qnn_circ, n_qubits)
    RY_layer(qnn_circ, params_list[n_qubits*k:(n_qubits*(k+1))])
...
readout_op = ListOp([~StateFn(PauliSumOp.from_list([('ZIII', 1)])) @ StateFn(qc),
                     ~StateFn(PauliSumOp.from_list([('IZII', 1)])) @ StateFn(qc),
                     ~StateFn(PauliSumOp.from_list([('IIZI', 1)])) @ StateFn(qc),
                     ~StateFn(PauliSumOp.from_list([('IIIZ', 1)])) @ StateFn(qc),
                            ] )
qnn = OpflowQNN(readout_op, preparation_circ.parameters, qnn_circ.parameters, expval, gradient, qi)
initial_weights = torch.ones((q_depth)*n_qubits, requires_grad=True)
...
#         self.q_params = nn.Parameter(q_delta * torch.randn(q_depth * n_qubits))
    ...
    def forward(self, input_features):
        ...
        for elem in q_in:
            ...
            q_out_elem = model(elem).float().unsqueeze(0)
...
np.savez("transfer_learning_qiskit.npz", t=np.array(t_list), a=np.array(acc_list))
